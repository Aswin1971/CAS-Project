...
public class NewsPage extends BasePage{
	...
	public void newsHeaders(WebDriver driver) throws InterruptedException{
		Thread.sleep(5000);
		boolean flag=true;
        while((headers.size()>0 && flag==true)){					//Collection of headers and toolip till end of the page
    		...
    		js.executeScript("arguments[0].scrollIntoView();",seventh);      //Scroll till the last element visibility
            Thread.sleep(5000);
	...
	public void newsDetailsPrint() throws IOException{
		int rowh=1;
        for(String h:headerslist) {						//Print the headers List data to excel
        ...
        int rowt=1;
        for(String t:tooltipslist) {				//Print the toolTip List data to excel
        ...
        int rowc=1; 
        for(String c:contentslist) {					//Print the Content List data to excel
	...
	public void FivenewsDetailsPrint() throws IOException{
		...
		int rowh=1;
        for(int i=0;i<5;i++) {		//Print the headers List data's to excel
        ...
        int rowt=1;
        for(int i=0;i<5;i++) {				//Print the toolTip List data to excel
        ...
        int rowc=1; 
        for(int i=0;i<5;i++) {					//Print the Content List data's to excel
